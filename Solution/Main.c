 ...
 * Otherwise, this code is set to run as a stand alone C program with ASCII display on a PC.
 */
 ...
 * If on Arduino, use the Arduino LED Matrix display.
 * If on a PC, use the ASCII console display.
 */
void displayBoard() {
#ifdef arduino
...
/**
 * Set the game identifier number.
 */
...
int takeAutoTurn() {
	...
	if (bestScore > 0) {
		...
		i--;
		// if game number = 0, play a truely random move.
		// otherwise, play a move using the game number as the randomizer seed.
		srand(gameNumber);
		int index = rand() % i;
		// start with original board state
		for (i = 0; i < 8; i++) {
